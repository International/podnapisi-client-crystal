...
begin
  ...
  language       = opts.language.downcase
rescue KeyError
  abort "Usage: -n|--name name -s|--season season_number -e|--episode episode_number -l|--language pl"
end

client  = Podnapisi::Client.new(show_name, season_number, episode_number, language)
...
unless results.empty?
  results.each_with_index do |res,idx|
    ...
    if opts.download?
      file_name = "#{show_name}S#{season_number}E#{episode_number}_sub_#{idx}.zip"
